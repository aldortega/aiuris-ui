...
<template>
  <aside class="h-full w-full">
    <nav class="h-full overflow-y-auto bg-white p-4">
      <h2 class="mb-3 text-sm font-semibold uppercase tracking-wide text-slate-500">Navegacion</h2>
      <ul class="space-y-1">
        <li v-for="item in items" :key="item.label">
          ...
          >
            <component
              :is="item.icon"
              class="h-5 w-5"
              :class="isActive(item) ? 'text-white' : 'text-slate-500'"
            />
            <span class="flex-1">{{ item.label }}</span>
          </RouterLink>
          <div
            v-else
            class="flex items-center gap-3 rounded-md px-3 py-2 text-sm font-medium text-slate-400"
            title="Proximamente"
          >
            <component :is="item.icon" class="h-5 w-5 text-slate-300" />
            <span class="flex-1">{{ item.label }}</span>
          </div>
        </li>
      </ul>
